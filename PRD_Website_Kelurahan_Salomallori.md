...
Proyek ini bertujuan untuk membangun **Website Profil Kelurahan Salomallori**, Kecamatan Dua Pitue, Kabupaten Sidenreng Rappang (Sidrap), Sulawesi Selatan. Website ini dikembangkan sebagai bagian dari program Kuliah Kerja Nyata (KKN) Universitas Hasanuddin. Platform ini akan menjadi pusat informasi digital kelurahan yang menampilkan profil kelurahan, potensi wisata, UMKM lokal, berita kegiatan, serta galeri dokumentasi. Dikembangkan menggunakan teknologi modern (Next.js, Tailwind CSS, Prisma, PostgreSQL) agar responsif, cepat, dan mudah dikelola. Selain itu, website juga dilengkapi modul **Tracking Pelayanan Kelurahan** untuk memudahkan warga mengajukan permohonan administrasi secara online maupun langsung di kantor kelurahan dengan sistem ticketing.
...
6. Mendigitalisasi pelayanan administrasi kelurahan dengan sistem ticketing dan tracking
...
| Pengguna | Kebutuhan |
|---|---|
| **Masyarakat Kelurahan** | Mendapatkan informasi kelurahan, berita, UMKM, galeri, mengajukan pelayanan administrasi via tracking |
| **Perangkat Kelurahan** | Mengelola konten website, memproses permohonan warga, mengelola layanan & form |
...
| No | Fitur | Deskripsi |
|---|---|---|
| 1 | **Landing Page / Beranda** | Hero section, statistik kelurahan, highlight potensi, berita terbaru, galeri |
| 2 | **Profil Kelurahan** | Sejarah, visi misi, data geografis & demografis, struktur pemerintahan |
| 3 | **Berita & Kegiatan** | Daftar artikel berita kelurahan dengan filter & pencarian, detail berita |
...
| 7 | **Kontak & Lokasi** | Google Maps, kontak WhatsApp, alamat kantor kelurahan |
| 8 | **Infografis** | Visualisasi data statistik kelurahan interaktif (demografi, dusun, mata pencaharian) |
| 9 | **IDM (Indeks Desa Membangun)** | Status IDM kelurahan, skor 3 pilar (sosial, ekonomi, ekologi), capaian SDGs |
...
| 6 | **Kelola Profil Kelurahan** | Edit data profil kelurahan, perangkat kelurahan |
| 7 | **Kelola Pengguna** | Manajemen admin/editor (Super Admin) |
| 8 | **Kelola Infografis** | Update data statistik kelurahan & IDM |
| 9 | **Kelola Layanan** | CRUD jenis pelayanan kelurahan (KK, Surat Tanah, dll.) + upload template file |
...
### 5.3 Modul Pelayanan Kelurahan — Tracking & Ticketing
...
| 3 | **Pengajuan Offline (Dari Kantor Kelurahan)** | Warga cukup klik "Buat Ticket Offline" → ticket kosong dengan nomor seri terbuat, admin yang memilihkan layanan & mengisi form |
...
**Nomor Tiket:** Format `SL-YYYYMMDD-XXX`, counter reset per hari.
...
| **Rich Text Editor** | TipTap 3 (ProseMirror) | WYSIWYG editor untuk konten berita & halaman |
| **Image/File Upload** | Cloudinary 2 | Storage gambar & template file |
| **Email** | Nodemailer + Gmail SMTP | Notifikasi email otomatis |
| **Form Validation** | Zod 4 | Validasi input form dinamis |
...
### 7.2 Model — Tracking Pelayanan (Fase 5)
...
```
Beranda → Lihat Profil Kelurahan → Lihat Berita → Detail Berita
...
*Dokumen ini disusun oleh Tim KKN Universitas Hasanuddin — Program Kerja Pembuatan Website Profil Kelurahan Salomallori, Kecamatan Dua Pitue, Kabupaten Sidenreng Rappang.*