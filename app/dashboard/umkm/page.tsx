...
function getKategoriColor(kategori: string): string {
  const colors: Record<string, string> = {
    ...
    kerajinan: "bg-amber-100 text-amber-700 dark:bg-amber-900/30 dark:text-amber-400",
    "jasa": "bg-blue-100 text-blue-700 dark:bg-blue-900/30 dark:text-blue-400",
    "teknologi": "bg-purple-100 text-purple-700 dark:bg-purple-900/30 dark:text-purple-400",
  };
  return colors[kategori.toLowerCase()] ?? "bg-foreground/10 text-foreground/70";
}
...
export default function UMKMListPage() {
  ...
  const [confirmItem, setConfirmItem] = useState<UMKM | null>(null);
  const debounceTimer = useRef<ReturnType<typeof setTimeout> | null>(null);

  // Deduplicate kategori list from fetched data
  ...
  const fetchData = useCallback(async () => {
    ...
    try {
      ...
      if (res.ok) {
        ...
        // Collect unique kategori
        const unik = new Set<string>();
        json.data.forEach((item: UMKM) => unik.add(item.kategori));
        setKategoriList((prev) => {
          const merged = new Set([...prev, ...unik]);
  ...
  async function handleDelete(id: string) {
    ...
    try {
      ...
      if (res.ok) {
        ...
        toast.error(body?.error ?? "Gagal menghapus UMKM");
        setConfirmItem(null);
      }
    } catch {
          ...
          <DialogHeader>
            ...
            <DialogDescription>
              ...
              <span className="font-semibold text-foreground">
                &ldquo;{confirmItem?.namaProduk}&rdquo;
              </span>
                    ...
                    <div className="flex flex-col items-center gap-3 py-12 text-muted-foreground">
                      ...
                      <p className="text-sm font-medium">
                        {debouncedSearch || kategoriFilter !== "all"
                          ...
                          : "Belum ada UMKM. Tambah yang pertama!"}
                      </p>
                      {!debouncedSearch && kategoriFilter === "all" && (
                  ...
                  <TableRow key={item.id}>
                    ...
                    <TableCell>
                      <div className="flex items-center justify-end gap-1">
                        ...
                        >
                          <Link href={`/dashboard/umkm/${item.id}/edit`}>
