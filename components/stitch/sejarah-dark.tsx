...
export function SejarahDark({ data }: { data: SejarahData }) {
  const stats = [
    {
      icon: Map,
      value:
  ...
  return (
    <div className="min-h-screen bg-[#111415] font-sans text-[#e1e3e0] antialiased transition-colors duration-300">
      ...
      {/* Sejarah */}
      <section id="sejarah" className="border-y border-[#373a3b] bg-[#191c1d] py-20">
        <div className="mx-auto grid max-w-6xl grid-cols-1 gap-12 lg:grid-cols-12">
          <div className="lg:col-span-7">
            <Reveal>
              <div className="mb-6 flex items-center gap-3">
                <div className="rounded-lg bg-[#373a3b] p-2">
                  <Compass className="h-5 w-5 text-[#84bd3a]" />
                </div>
            ...
            <Reveal delay={100}>
              <p className="text-[16px] leading-[1.9] text-[#b0b4b5]">
                {sejarah}
              </p>
            </Reveal>
          </div>
            ...
            <Reveal delay={200}>
              <div className="rounded-xl border border-[#373a3b] bg-[#111415] p-6 shadow-sm transition-colors duration-300 hover:border-[#32735f]/60">
                <div className="mb-4 flex items-center gap-3">
                  <div className="rounded-lg bg-[#373a3b] p-2">
                    <MapPin className="h-5 w-5 text-[#84bd3a]" />
                  </div>
      ...
      <section className="mx-auto max-w-4xl px-6 py-20 text-center">
        <Reveal>
          <div className="mx-auto mb-6 flex h-14 w-14 items-center justify-center rounded-full bg-[#191c1d] border border-[#373a3b]">
            <span className="font-serif text-3xl text-[#febe0d]">&ldquo;</span>
          </div>
          ...
          <p className="mt-6 text-sm font-semibold tracking-wide text-[#b0b4b5]">
            — Visi Kelurahan {nama}
          </p>
        </Reveal>
      </section>
      ...
      {/* Visi Misi */}
      <section className="border-t border-[#373a3b] bg-[#191c1d] py-20">
        <div className="mx-auto max-w-6xl px-6">
          ...
          <div className="grid grid-cols-1 gap-8 md:grid-cols-2">
            <Reveal delay={100}>
              <div className="h-full rounded-[12px] border border-[#373a3b] bg-[#111415] p-8 shadow-sm transition-all duration-300 hover:-translate-y-1 hover:border-[#32735f]/60 hover:shadow-lg hover:shadow-black/20">
                ...
                <p className="text-[15px] leading-relaxed text-[#b0b4b5]">
                  {data.desa?.visi}
                </p>
              </div>
            </Reveal>
                  ...
                  {data.desa?.misi
                    ? data.desa.misi
                        ...
                        .map((line, index) => (
                          <p
                            key={index}
                            className="flex items-start gap-3 text-[15px] leading-relaxed text-[#b0b4b5]"
                          >
                            <span className="mt-1.5 flex h-2.5 w-2.5 shrink-0 rounded-full bg-[#84bd3a]" />
                            {line.replace(/^[-•]\s*/, "")}
                          </p>
                        ))
                    : null}
      ...
      <section className="mx-auto max-w-6xl px-6 py-20">
        <Reveal>
          <div className="flex flex-col items-center justify-between gap-6 rounded-[16px] border border-[#373a3b] bg-[#191c1d] p-10 text-center md:flex-row md:text-left">
            <div>
            ...
            <div className="flex shrink-0 flex-wrap justify-center gap-4">
              <Link
                href="/news"
                className="inline-flex h-11 items-center gap-2 rounded-[4px] bg-[#0b2b40] px-6 text-sm font-semibold text-white transition-all duration-200 hover:-translate-y-0.5 hover:bg-[#061824]"
              >
                Lihat Berita <ArrowRight className="h-4 w-4" />
              </Link>
              <a
                href="/galeri"
                className="inline-flex h-11 items-center gap-2 rounded-[4px] border border-[#373a3b] bg-transparent px-6 text-sm font-semibold text-white transition-all duration-200 hover:-translate-y-0.5 hover:border-[#84bd3a] hover:text-[#84bd3a]"
              >
                Buka Galeri
              </a>
            </div>
